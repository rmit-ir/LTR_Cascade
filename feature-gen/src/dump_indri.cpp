...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cerr << "USAGE: " << argv[0] << " <indri repository> <newt collection folder>"
                  << std::endl;
        return EXIT_FAILURE;
    }
    ...
    of_globalinfo << index->documentCount() << " " << index->termCount() << std::endl;
    ...
    // write dictionary
    {
        ...
        iter->startIteration();
        std::cerr << "DIC TOTAL"
                  << " " << index->termCount() << " " << index->documentCount() << std::endl;
        ...
        while (!iter->finished()) {
            ...
            of_dict << termData->term << " " << i << " " << termData->corpus.documentCount << " "
