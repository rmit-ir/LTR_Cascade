#ifndef QUERY_ENVIRONMENT_ADAPTER_HPP
#define QUERY_ENVIRONMENT_ADAPTER_HPP
...
public:
  ...
  virtual double expression_count(const std::string &expr) {
    return env->expressionCount(expr);
  }

  virtual std::string stem_term(const std::string &term) {
    return env->stemTerm(term);
  }
  ...
  document_metadata(const std::vector<docid_t> &doc_ids,
                    const std::string &attribute_name) {
    return env->documentMetadata(doc_ids, attribute_name);
  }

  virtual std::vector<docid_t>
  document_ids_from_metadata(const std::string &name,
  ...
  virtual std::vector<indri::api::ParsedDocument *>
  documents(const std::vector<docid_t> &doc_ids) {
