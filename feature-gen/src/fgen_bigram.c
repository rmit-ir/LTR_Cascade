...
uint32_t *build_doclen(char *dfile, int *ndocs, double *avg_dlen) {
    ...
    while (fscanf(docfile, "%" SCNu32 ":", &len) == 1) {
        dl_sum += len;
        if (dmsize == *ndocs) {
            uint32_t *tmpa = (uint32_t *)safe_realloc(dlen, dmsize * 2 * sizeof(uint32_t));
            dlen           = tmpa;
            dmsize *= 2;
        }
        dlen[*ndocs] = len;
...
double calculate_bm25(uint32_t dlen, double avg_dlen, uint32_t ndocs, uint32_t t_idf, uint32_t d_f)
...
double calculate_tfidf(double d_f, double t_idf, double dlen, uint32_t num_docs) {
...
double calculate_prob(uint32_t d_f, uint32_t dlen) { return ((double)d_f / dlen); }

double calculate_bose_ein(
    uint32_t d_f, uint64_t c_f, uint32_t num_docs, double avg_dlen, uint32_t dlen) {
...
double calculate_dph(
    uint32_t d_f, uint64_t c_f, uint32_t num_docs, double avg_dlen, uint32_t dlen) {
    double f, norm, score, one_minus_f;
    f = (double)d_f / (double)dlen;

    // prevent log nan, for some bigram postings
    one_minus_f = 1.0 - f;
    if (one_minus_f < 0) {
        one_minus_f = 1e-6;
    }
    ...
    score = 1.0 * norm *
            ((double)d_f * log2(((double)d_f * (double)avg_dlen / (double)dlen) *
             ...
             0.5 * log2(2.0 * M_PI * d_f * one_minus_f));
    return (score);
}
...
void compute_prob_stats(
    feature_t *f, uint32_t *doclen, posting_t *posting, uint64_t pos, double *max) {
    uint64_t i;
    uint32_t size = pos;
    uint32_t mid  = size / 2;
    uint32_t lq   = size / 4;
...
void compute_be_stats(feature_t *f,
    ...
    for (i = 0; i < pos; i++) {
        double score = 0.0;
        score =
            calculate_bose_ein(posting[i].freq, c_f, ndocs, avg_dlen, doclen[posting[i].docid - 1]);
        bmtmp[i] = score;
        if (score > *max)
            *max = score;
    }
...
void compute_dph_stats(feature_t *f,
    ...
    for (i = 0; i < pos; i++) {
        double score = 0.0;
        score = calculate_dph(posting[i].freq, c_f, ndocs, avg_dlen, doclen[posting[i].docid - 1]);
        bmtmp[i] = score;
        if (score > *max)
            *max = score;
    }
...
void compute_dfr_stats(feature_t *f,
    ...
    for (i = 0; i < pos; i++) {
        double score = 0.0;
        score =
            calculate_dfr(posting[i].freq, c_f, pos, ndocs, avg_dlen, doclen[posting[i].docid - 1]);
        bmtmp[i] = score;
        if (score > *max)
            *max = score;
    }
...
void compute_tfidf_stats(
    feature_t *f, uint32_t *doclen, posting_t *posting, uint64_t pos, uint64_t ndocs, double *max) {
    uint64_t i;
    uint32_t size = pos;
    uint32_t mid  = size / 2;
    uint32_t lq   = size / 4;
    ...
    for (i = 0; i < pos; i++) {
        ...
        score    = calculate_tfidf(posting[i].freq, pos, doclen[posting[i].docid - 1], ndocs);
        bmtmp[i] = score;
        if (score > *max)
            *max = score;
    }
...
void compute_bm25_stats(feature_t *f,
    ...
    for (i = 0; i < pos; i++) {
        double score = 0.0;
        score = calculate_bm25(doclen[posting[i].docid - 1], avg_dlen, ndocs, pos, posting[i].freq);
        bmtmp[i] = score;
        if (score > *max)
            *max = score;
    }
...
void compute_lm_stats(feature_t *f,
    ...
    for (i = 0; i < pos; i++) {
        double score = 0.0;
        score        = calculate_lm(posting[i].freq, cf, doclen[posting[i].docid - 1], clen);
        bmtmp[i]     = score;
        if (score > *max)
            *max = score;
    }
...
void print_feature(feature_t *f) {
    fprintf(stdout, "%s ", f->term_a);
    fprintf(stdout, "%s ", f->term_b);
