#ifndef DOC_TPSCORE_FEATURE_HPP
#define DOC_TPSCORE_FEATURE_HPP
...
struct bctp_document {
  ...
  void set_positions(indri::utility::greedy_vector<int> _positions) {
    positions.clear();
...
struct bctp_scorer {
  ...
  inline double distance(size_t pos_i, size_t pos_j) {
    return std::pow(pos_j - pos_i, -2);
  }
};
...
public:
  ...
  void compute(fat_cache_entry &doc, std::vector<std::string> &query_stems, freqs_entry &freqs) {
    ...
    bctp_doc.set_positions(doc.term_list->terms());
    ...
    for (auto &s : query_stems) {
      ...
      q.id = index.term(s);
      q.doc_count = doc_data_helper->get_term_frequency(doc.id, q.id, index);
      bctp_query.push_back(q);
    }
