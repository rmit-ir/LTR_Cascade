...
static void delete_term(term_t *data) {
  if (data->term) {
    free(data->term);
  }
  free(data);
...
void destroy_termhash(termhash_t *termhash) {
  ...
  for (i = 0; i < termhash->buckets; i++) {
    if ((value = termhash->array[i])) {
      delete_term(value);
    }
  }
  free(termhash->array);
...
static size_t get_term_pos(termhash_t *termmap, char *buf, size_t *pos) {
  term_t *value;
...
static void check_termhash(termhash_t *termmap) {
  size_t i;
  size_t old_buckets;
  term_t **old_array;
  ...
  for (i = 0; i < old_buckets; i++) {
    ...
    if (value) {
      size_t pos;
      get_term_pos(termmap, (char *)value->term, &pos);
...
term_t *find_term(termhash_t *termmap, char *buf) {
  ...
  found = get_term_pos(termmap, buf, &pos);
  if (!found) {
    return (NULL);
  }
...
void add_term(termhash_t *termmap, term_t *buf) {
  ...
  check_termhash(termmap);

  found = get_term_pos(termmap, buf->term, &pos);
...
termhash_t *load_termmap(const char *fname) {
  ...
  while (
      fscanf(
          input, "%s %" SCNu64 " %" SCNu64
                 " %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf "
                 "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf "
                 "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf "
                 "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf "
                 "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",
          d_str, &cf, &cdf, &geo_mean, &bm25_median_score, &bm25_firstq_score,
          &bm25_thirdq_score, &bm25_max_score, &bm25_min_score,
          &bm25_mean_score, &bm25_score_variance, &bm25_score_stddev,
          &bm25_score_confidence, &bm25_score_harmonic_mean, &tf_median_score,
          &tf_firstq_score, &tf_thirdq_score, &tf_max_score, &tf_min_score,
          &tf_mean_score, &tf_score_variance, &tf_score_stddev,
          &tf_score_confidence, &tf_score_harmonic_mean, &lm_median_score,
          &lm_firstq_score, &lm_thirdq_score, &lm_max_score, &lm_min_score,
          &lm_mean_score, &lm_score_variance, &lm_score_stddev,
          &lm_score_confidence, &lm_score_harmonic_mean, &pr_median_score,
          &pr_firstq_score, &pr_thirdq_score, &pr_max_score, &pr_min_score,
          &pr_mean_score, &pr_score_variance, &pr_score_stddev,
          &pr_score_confidence, &pr_score_harmonic_mean, &be_median_score,
          &be_firstq_score, &be_thirdq_score, &be_max_score, &be_min_score,
          &be_mean_score, &be_score_variance, &be_score_stddev,
          &be_score_confidence, &be_score_harmonic_mean, &dph_median_score,
          &dph_firstq_score, &dph_thirdq_score, &dph_max_score, &dph_min_score,
          &dph_mean_score, &dph_score_variance, &dph_score_stddev,
          &dph_score_confidence, &dph_score_harmonic_mean, &dfr_median_score,
          &dfr_firstq_score, &dfr_thirdq_score, &dfr_max_score, &dfr_min_score,
          &dfr_mean_score, &dfr_score_variance, &dfr_score_stddev,
          &dfr_score_confidence, &dfr_score_harmonic_mean) == 74) {
    term_t *curr = find_term(map, d_str);
    if (curr != NULL) {
...
static size_t get_stopword_pos(stophash_t *termmap, const char *buf,
                               size_t *pos) {
  stopword_t *value;
  *pos = murmur_hash(buf, termmap->buckets);
...
static size_t get_queryterm_pos(queryhash_t *termmap, const char *buf,
                                size_t *pos) {
  queryterm_t *value;
  *pos = murmur_hash(buf, termmap->buckets);
...
char *fgen_term_qry_main(termhash_t *termmap, int qnum, char **termv,
                         size_t termc) {
  int tcnt = 0, i = 0;
