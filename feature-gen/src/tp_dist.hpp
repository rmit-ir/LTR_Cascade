//
// Created by xiaolul on 4/25/16.
//

#ifndef WANDBL_THETA_TP_DIST_HPP
#define WANDBL_THETA_TP_DIST_HPP
...
public:
  ...
  calc_tp_dist(const indri::utility::greedy_vector<int> &pos_i,
               const indri::utility::greedy_vector<int> &pos_j,
               const double w_i, const double w_j, int wsize) {
    std::pair<double, double> dist_scores(0.0, 0.0);
    std::pair<uint64_t, uint64_t> prev_rhs(0, 0);
    indri::utility::greedy_vector<int>::const_iterator curr_itrs[] = {
        pos_i.begin(), pos_j.begin()};
    indri::utility::greedy_vector<int>::const_iterator end_itrs[] = {
        pos_i.end(), pos_j.end()};
    TermPos lhs, rhs;
...
protected:
  static void _acc_dist(TermPos lhs, TermPos rhs, double w_i, double w_j,
                        std::pair<double, double> &acc_dist,
                        std::pair<uint64_t, uint64_t> &prev_pos, size_t wsize) {
    if (rhs.m_pos - lhs.m_pos + 1 > wsize)
      return;
    double dist_weight =
        _calculate_dist_weight(w_i, w_j, (rhs.m_pos - lhs.m_pos + 1));
...
private:
  static double _calculate_dist_weight(double w_i, double w_j, uint64_t dist) {
    double dist_weight =
        (std::min(w_i, 1.0) + std::min(w_j, 1.0)) / static_cast<double>(dist);
    dist_weight = w_i * w_j * dist_weight * dist_weight;
    return dist_weight;
  }
};

#endif // WANDBL_THETA_TP_DIST_HPP
