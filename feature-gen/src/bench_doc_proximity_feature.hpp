...
struct term_data {
  ...
  term_data(int id, uint64_t term_count, size_t freq_dt, double idf_weight,
            int qpos)
      : tid(id), total_term_docs(term_count), f_dt(freq_dt), w_q(idf_weight),
        query_pos(qpos) {}
};

class bench_doc_proximity_feature {
...
public:
  bench_doc_proximity_feature(indri_index &idx) : index(idx) {
  ...
  void compute_cdf_bigram(fat_cache_entry &doc, query_train &query) {
    score = 0.0;
    ...
    for (auto &tid : query.tids) {
      ...
      if (tid > 0) {
        ...
        if (doc_data && doc_data->document == doc.id) {
          ...
          term_data curr_term(tid, index.documentCount(index.term(tid)),
          ...
          /* _acc_positions_insert(acc_positions, doc_data->positions,
           * acc_terms, */
          /*                       curr_term); */
    ...
    // sort cdf
    std::sort(cdf.begin(), cdf.end(), [](const std::pair<uint64_t, int> &lhs,
                                         const std::pair<uint64_t, int> &rhs) {
      return lhs.second < rhs.second;
    });

    // find bigrams of all query term pairs in uw8
    cdf_search(TP_BIGRAM, cdf, query, 8, doc.length);
    doc.bm25_bigram_u8 = score;

    // Xiaolu, et al.
    /* doc.bm25_tp_dist_w100 = */
    /*     tp_interval_score(acc_positions, acc_terms, 100, doc.length); */

    // Clear for next doc
    term_data_map.clear();
  }

  void compute_tp_interval(fat_cache_entry &doc, query_train &query) {
    score = 0.0;
    ...
    for (auto &tid : query.tids) {
      ...
      if (tid > 0) {
        ...
        if (doc_data && doc_data->document == doc.id) {
          ...
          _acc_positions_insert(acc_positions, doc_data->positions, acc_terms,
                                curr_term);
  ...
  struct term_pair {
    ...
    term_pair(enum term_pair_type ty, int dist, std::pair<int, int> a,
              std::pair<int, int> b, double s)
  ...
  void cdf_search(const enum term_pair_type ty,
                  ...
                  const query_train &query, const int window,
    ...
    for (size_t i = 0; i < cdf.size() - 1; ++i) {
      ...
      for (size_t j = i + 1; j < cdf.size(); i = j++) {
        ...
        if (dist > 0 && dist <= _window) {
          ...
          score += ranker.score(q_ft, data_a.f_dt, data_a.total_term_docs,
                                doc_length);
          ...
          score += ranker.score(q_ft, data_b.f_dt, data_b.total_term_docs,
                                doc_length);
   ...
   */
  double tp_interval_score(
      const std::vector<indri::utility::greedy_vector<int>> &acc_positons,
      const std::vector<term_data> &acc_terms, const int wsize,
      const double W_d) {
    ...
    for (size_t i = 0; i < acc_positons.size() - 1; ++i) {
      ...
      for (size_t j = (i + 1); j < acc_positons.size(); ++j) {
        ...
        if (std::abs(delta_order) == 1) {
          ...
          if (delta_order > 0) {
            curr_score = TPDist::calc_tp_dist(i_position, j_position,
                                              term_i.w_q, term_j.w_q, wsize);
          } else {
            curr_score = TPDist::calc_tp_dist(j_position, i_position,
                                              term_j.w_q, term_i.w_q, wsize);
          }
          curr_score.first =
              lambda_o * ranker.calculate_tf_score(curr_score.first, W_d);
          curr_score.second =
   ...
   */
  void _acc_positions_insert(
      std::vector<indri::utility::greedy_vector<int>> &pos_vec,
      indri::utility::greedy_vector<int> &pos_el,
      std::vector<term_data> &term_vec, term_data &term_el) {
  ...
  void set_term_data_map(const std::map<uint64_t, term_data> &data) {
    term_data_map = data;
  }
